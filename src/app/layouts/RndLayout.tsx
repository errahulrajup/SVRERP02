...
export function RndLayout() {
  ...
  return (
    <div className="rnd-app" style={{ display: 'flex', minHeight: '100vh', background: '#0C1510', color: '#F0EDE6', fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}>
      ...
      <style>{`
        .rnd-sidebar {
          width: 240px; background: #111C14; border-right: 1px solid rgba(123,169,123,0.12);
          display: flex; flex-direction: column; transition: transform 0.3s ease; z-index: 200;
        }
        .rnd-nav-link {
          display: flex; align-items: center; gap: 12px; padding: 10px 16px;
          background: transparent; border: none; width: 100%; text-align: left;
          color: #9AAF96; font-size: 13px; font-weight: 500; cursor: pointer;
          transition: all 0.2s; border-radius: 6px; margin-bottom: 3px;
        ...
        .rnd-card { background: #111C14; border: 1px solid rgba(123,169,123,0.12); border-radius: 12px; padding: 20px; }
        .rnd-card-header { font-size: 13px; font-weight: 600; color: #9AAF96; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 16px; border-bottom: 1px solid rgba(123,169,123,0.12); padding-bottom: 12px; }

        .rnd-btn { background: #1C2A1E; color: #F0EDE6; border: 1px solid rgba(123,169,123,0.18); padding: 8px 16px; border-radius: 6px; font-size: 13px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
        ...
        .rnd-input { background: #111C14; border: 1px solid rgba(123,169,123,0.22); color: #F0EDE6; padding: 8px 12px; border-radius: 6px; font-size: 13px; outline: none; transition: border-color 0.2s; }
        ...
        .rnd-table th { text-align: left; padding: 10px 14px; border-bottom: 1px solid rgba(123,169,123,0.12); color: #556355; font-weight: 600; text-transform: uppercase; font-size: 10px; letter-spacing: 0.08em; }
        .rnd-table td { padding: 11px 14px; border-bottom: 1px solid rgba(123,169,123,0.08); color: #9AAF96; }
        .rnd-table tr:hover td { background: rgba(212,168,67,0.04); }

        .rnd-badge { display: inline-flex; align-items: center; padding: 2px 8px; border-radius: 999px; font-size: 11px; font-weight: 600; text-transform: uppercase; }
      ...
      <aside className={`rnd-sidebar${open ? ' open' : ''}`}>
        <div style={{ padding: '20px 16px 14px', borderBottom: '1px solid rgba(123,169,123,0.12)', marginBottom: 8 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#F0EDE6', letterSpacing: '-0.3px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 30, height: 30, background: 'rgba(212,168,67,0.1)', border: '1px solid rgba(212,168,67,0.25)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
              ⌬
            </div>
            Srivriddhi R&D
          </div>
          <div style={{ fontSize: 9, color: '#A07828', letterSpacing: '2px', textTransform: 'uppercase', marginTop: 3, fontWeight: 600 }}>
            Laboratory Portal
          </div>
        </div>
        ...
        <div style={{ padding: '12px', borderTop: '1px solid rgba(123,169,123,0.12)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: '#0C1510', borderRadius: 8, border: '1px solid rgba(123,169,123,0.12)' }}>
            <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#1C2A1E', border: '1px solid rgba(123,169,123,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13 }}>
              👤
            </div>
            <div style={{ flex: 1, overflow: 'hidden' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#F0EDE6', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>{role || 'Scientist'}</div>
              <div style={{ fontSize: 10, color: '#556355' }}>Formulation Lab</div>
            </div>
