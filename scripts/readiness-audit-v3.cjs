...
// Helper to retrieve database execution time using EXPLAIN ANALYZE
async function getQueryExecutionTime(sql) {
  try {
    const res = await runSQL(`EXPLAIN (ANALYZE, FORMAT JSON) ${sql}`);
    const rawPlan = res[0]?.['QUERY PLAN'] || res[0]?.['query plan'];
    const parsedPlan = typeof rawPlan === 'string' ? JSON.parse(rawPlan) : rawPlan;
    // Execution Time is returned in milliseconds
    return parsedPlan[0]?.['Execution Time'] || 0;
  } catch (e) {
    console.error(`Failed to analyze execution time for query: ${e.message}`);
    return -1;
  }
}
...
async function getTableStateHash(tableName) {
  try {
    console.log(`[Hash Check] Querying hash for ${tableName}...`);
    const query = `SELECT count(*)::text || '-' || coalesce(sum(hashtext(id::text)), 0)::text as hash FROM ${tableName};`;
    const res = await runSQL(query);
    console.log(`[Hash Check] Hash for ${tableName}: ${res[0]?.hash || 'empty'}`);
    return res[0]?.hash || 'empty';
  } catch (e) {
    console.log(`[Hash Check] Error querying ${tableName}: ${e.message}`);
...
async function main() {
  ...
  console.log('[main] Capturing baseline hashes...');
  ...
  console.log('[main] Baseline hashes captured.');
  ...
  try {
    ...
    console.log('⚡ Recompiling inv.allocate_fefo with #variable_conflict use_column...');
    await runSQL(`
      ...
      $$;
    `);
    console.log('✅ Function inv.allocate_fefo successfully recompiled.');
  } catch (e) {
    console.error('❌ Failed during Phase 0 setup:', e.message);
  }
  ...
  await assertSQL(
    ...
    DELETE FROM inv.movements WHERE lot_id IN (SELECT id FROM inv.lots WHERE lot_code LIKE 'LOT-FEFO-%');
    ...
    VALUES ('77777777-7777-7777-7777-777777777777', 'abf5996a-ead9-4307-b0a7-5caa30e52e9c', 'FEFO Test Item', 'ITEM-FEFO-TEST', 'RAW_MATERIAL', 10.0);
    ...
    -- Use SELECT instead of PERFORM to avoid raw SQL query syntax errors
  ...
  await assertSQL(
    ...
    DELETE FROM public.lots WHERE lot_no = 'LOT-TRACE-01' OR id = '30000000-0000-0000-0000-000000000001';
    DELETE FROM public.grns WHERE grn_no = 'GRN-TRACE-01' OR id = '40000000-0000-0000-0000-000000000001';
    DELETE FROM md.suppliers WHERE code = 'SUPP-TRACE-01' OR id = '20000000-0000-0000-0000-000000000001';
    DELETE FROM md.items WHERE code = 'ITEM-TRACE-01' OR id = '10000000-0000-0000-0000-000000000002';

    INSERT INTO md.suppliers (id, org_id, name, code)
    VALUES ('20000000-0000-0000-0000-000000000001', 'abf5996a-ead9-4307-b0a7-5caa30e52e9c', 'Traceability Supplier', 'SUPP-TRACE-01');

    INSERT INTO md.items (id, org_id, name, code, item_type)
    VALUES ('10000000-0000-0000-0000-000000000002', 'abf5996a-ead9-4307-b0a7-5caa30e52e9c', 'Traceability Item', 'ITEM-TRACE-01', 'RAW_MATERIAL');

    INSERT INTO public.grns (id, org_id, grn_no, supplier, material, qty, rate, total_cost, status)
    VALUES ('40000000-0000-0000-0000-000000000001', 'abf5996a-ead9-4307-b0a7-5caa30e52e9c', 'GRN-TRACE-01', 'Traceability Supplier', 'Traceability Item', 100.0, 10.0, 1000.0, 'QC_DONE');

    -- Mapping correctly to public.lots.lot_no column
    INSERT INTO public.lots (id, org_id, lot_no, material, qty, rate, remaining_qty, qc_status, supplier, total_cost, grn_id)
    VALUES ('30000000-0000-0000-0000-000000000001', 'abf5996a-ead9-4307-b0a7-5caa30e52e9c', 'LOT-TRACE-01', 'Traceability Item', 100.0, 10.0, 100.0, 'approved', 'Traceability Supplier', 1000.0, '40000000-0000-0000-0000-000000000001');
    ...
    INSERT INTO public.batch_components (batch_id, lot_id, material, actual_qty, planned_qty, org_id)
    VALUES ('B-TRACE-01', '30000000-0000-0000-0000-000000000001', 'Traceability Item', 50.0, 50.0, 'abf5996a-ead9-4307-b0a7-5caa30e52e9c');
  ...
  if (!perfOrgId2) {
    ...
    console.log('⚡ Running VACUUM ANALYZE to optimize indexes and visibility map...');
    await runSQL('VACUUM ANALYZE public.stock_ledger;');
    await runSQL('VACUUM ANALYZE public.batches;');
    await runSQL('VACUUM ANALYZE public.lots;');
    await runSQL('VACUUM ANALYZE public.grns;');
    await runSQL('VACUUM ANALYZE public.batch_components;');
    console.log('✅ Vacuum completed.');

    const ledgerSearchTime = await getQueryExecutionTime(`SELECT count(*) FROM public.stock_ledger WHERE org_id = '${perfOrgId2}'::uuid;`);
    const ledgerPass = ledgerSearchTime >= 0 && ledgerSearchTime < 1000;
    console.log(`⏱️ Stock Ledger Search Time: ${ledgerSearchTime.toFixed(2)}ms (SLA < 1s) -> ${ledgerPass ? '✅ PASS' : '❌ FAIL'}`);
    report.push({ name: 'Stock Ledger SLA Search Time', status: ledgerPass ? 'PASS' : 'FAIL', detail: `${ledgerSearchTime.toFixed(2)}ms` });

    const batchCostingTime = await getQueryExecutionTime(`SELECT count(*) FROM public.batches WHERE org_id = '${perfOrgId2}'::uuid;`);
    const costingPass = batchCostingTime >= 0 && batchCostingTime < 5000;
    console.log(`⏱️ Batch Costing Load Time: ${batchCostingTime.toFixed(2)}ms (SLA < 5s) -> ${costingPass ? '✅ PASS' : '❌ FAIL'}`);
    report.push({ name: 'Batch Costing SLA Load Time', status: costingPass ? 'PASS' : 'FAIL', detail: `${batchCostingTime.toFixed(2)}ms` });

    const traceabilityTime = await getQueryExecutionTime(`SELECT * FROM public.get_batch_consumed_lots('BATCH-PERF-100');`);
    const tracePass = traceabilityTime >= 0 && traceabilityTime < 2000;
    console.log(`⏱️ Traceability Query Time: ${traceabilityTime.toFixed(2)}ms (SLA < 2s) -> ${tracePass ? '✅ PASS' : '❌ FAIL'}`);
    report.push({ name: 'Traceability Query SLA Time', status: tracePass ? 'PASS' : 'FAIL', detail: `${traceabilityTime.toFixed(2)}ms` });
